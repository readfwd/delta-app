...
public class XAPKReader extends CordovaPlugin {
    ...
    private boolean triedToGetExpansionFile = false;
        ...
        if (action.equals("get")) {
            ...
            cordova.getThreadPool().execute(new Runnable() {
                public void run() {
                    try {
                        // Read file
                        PluginResult result = readFile(ctx, filename, mainVersion, patchVersion, PluginResult.MESSAGE_TYPE_ARRAYBUFFER);
    ...
    private PluginResult readFile(Context ctx, String filename, int mainVersion, int patchVersion, final int resultType) throws IOException {
        ...
        try {
            ...
            Log.e(LOG_TAG, "Can't retrieve file " + filename + ": " + ex.getLocalizedMessage());
        }
